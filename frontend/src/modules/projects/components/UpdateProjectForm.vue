...
const emit = defineEmits(['update:modalOpen', 'update']);
...
watch(
   ...
   (isOpen) => {
      if (isOpen) {
         localName.value = props.prevName;
         localDescription.value = props.prevDescription;
      }
   }
);
...
<div class="tw-mb-5">
   ...
   <div class="tw-mt-1 tw-text-right tw-text-xs tw-text-pulse-grayText">
      {{ localName.length }}/{{ projectNameLimit }}
   </div>
</div>
...
<div class="tw-mb-6">
   ...
   <div class="tw-mt-1 tw-text-right tw-text-xs tw-text-pulse-grayText">
      {{ localDescription.length }}/{{ projectDescriptionLimit }}
   </div>
</div>
            <!-- Actions -->
